...
# Build API 21 variant
build/$(APK_API21): $(BUILD_GRADLE).api21 $(foreach arch,$(ARCHES),$(APP_JNI_DIR)/$(arch)/$(GADGET_JNI_LIB)) | build
	@echo "Building API 21 variant..."
	@cp $(BUILD_GRADLE).api21 $(BUILD_GRADLE)
	docker run -it -u $(CURRENT_UID_GID) -v "$(ROOT_DIR)"/android_app:/app -w /app $(BUILD_IMAGE) $(GRADLE_BUILD)
	@cp $(APK_PATH) $@

# Build API 24 variant
build/$(APK_API24): $(BUILD_GRADLE).api24 $(foreach arch,$(ARCHES),$(APP_JNI_DIR)/$(arch)/$(GADGET_JNI_LIB)) | build
	@echo "Building API 24 variant..."
	@cp $(BUILD_GRADLE).api24 $(BUILD_GRADLE)
	docker run -it -u $(CURRENT_UID_GID) -v "$(ROOT_DIR)"/android_app:/app -w /app $(BUILD_IMAGE) $(GRADLE_BUILD)
	@cp $(APK_PATH) $@
